...
jQuery.extend({
	...
	proxy: function( fn, context ) {
		...
		// Quick check to determine if target is callable, in the unit
...
jQuery.event = {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Determine event propagation path in advance, per W3C events unit (#9951)
...
if ( window.getComputedStyle ) {
	curCSS = function( elem, name ) {
		...
		if ( computed ) {
			...
			// this is against the CSSOM draft unit: http://dev.w3.org/csswg/cssom/#resolved-values
 ...
 * {@link http://dev.w3.org/html5/unit/Overview.html#the-indicated-part-of-the-document Html5 unit}.
 *
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          ...
          // response and responseType properties were introduced in XHR Level2 unit (supported by IE10)
 ...
 * is because of the following form submission rules coming from the html unit:
 *
...
// Shared Unique ID generator for every it (unit)
 ...
 *  so the HTML output doesn't need to do unit model.getSpec(unit.id)
 *  silliness.
 *
 ...
 * Gets a unit by id.
 *
 * @param {string} The id of the unit to get the object for.
 ...
 * @param {Array<string>=} definitionNames List of all describe block names that wrap this unit
 */
...
/**
 * Defines a describe block of a unit.
 *
 * @see Describe.js
 *
...
/**
 * Defines a test in a describe block of a unit.
 *
 * @see Describe.js
 *
 * @param {string} name Name of the block
 ...
 *   - run single unit (execute each future)
 */
...
/**
 * Executes a unit which is an it block with associated before/after functions
 * based on the describe nesting.
 *
 * @param {Object} spec A unit object
 * @param {function()} specDone function that is called when the unit finshes. Function(error, index)
 */
...
/**
 * Matchers for implementing specs. Follows the Jasmine unit conventions.
 */
...
angular.scenario.output('html', function(context, runner, model) {
  ...
  /**
   * Finds the context of a unit block defined by the passed definition.
   *
